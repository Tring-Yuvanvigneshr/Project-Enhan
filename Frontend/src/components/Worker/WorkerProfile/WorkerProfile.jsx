...
const WorkerProfile = () => {
    ...
    useEffect(() => {
        if (data?.workerForworker) {
            ...
            setOriginalMobile(data.workerForworker.phone);
        }
    }, [data]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.phone !== originalMobile && !otpSend) {
            toast.error("Please send OTP first!");
            return;
        }

        const isOtpValid = formData.phone !== originalMobile && await verifyOtp(`+91${formData.phone}`, formData.otp);

        if (formData.phone !== originalMobile && !isOtpValid) {
